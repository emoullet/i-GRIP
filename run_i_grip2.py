...
from i_grip import SceneRenderer as sr
...
def scene_analysis_task(cam_data, stop_event, hands_queue, object_estimation_queue, hands_rendering_queue, objects_rendering_queue):
    plotter = pl.NBPlot()
    ...
    while True:
        ...
        if estimated_hands is not None:
            ...
            # if not hands_rendering_queue.empty():
            #     hands_rendering_queue.get()
            # hands_rendering_queue.put(scene.get_hands_rendering_data())
        ...
        if estimated_objects is not None:
            ...
            # if not objects_rendering_queue.empty():
            #     objects_rendering_queue.get()
            # objects_rendering_queue.put(scene.get_objects_rendering_data())
            # print(f'updated estimated objects')
        

def scene_rendering_task(stop_event, detect_event, img_queue, hands_data_queue, objects_data_queue   ):
    ...
    while True:
        ...
        if not hands_data_queue.empty():
            hands_data = hands_data_queue.get()
            renderer.set_hands_data(hands_data)
        if not objects_data_queue.empty():
            objects_data = objects_data_queue.get()
            renderer.set_objects_data(objects_data)
...
class GraspingDetector:
    ...
    def run(self):
        ...
        queue_hands_rendering = multiprocessing.Queue(maxsize=1)
        queue_objects_rendering = multiprocessing.Queue(maxsize=1)
        ...
        process_scene_analysis = multiprocessing.Process(target=scene_analysis_task, 
                                                        args=(cam_data, stop_event, queue_hands, queue_object_estimation, queue_hands_rendering, queue_objects_rendering,))
        
        process_scene_rendering = multiprocessing.Process(target=scene_rendering_task, 
                                                        args=(stop_event, detect_event, queue_rgb_frame_scene_analysis, queue_hands_rendering, queue_objects_rendering))
        
        process_hands_detection.start()
        process_object_detection.start()
        process_object_estimation.start()
        process_scene_analysis.start()
        # process_scene_rendering.start()
        rgbd_cam.start()
