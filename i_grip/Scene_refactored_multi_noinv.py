...
from i_grip.Objects_noinv import RigidObject
from i_grip.Targets_refactored_multi_and_thread_noinv import TargetDetector
...
class Scene :
    ...
    def update_object_meshes(self, scene):     
        objects_to_delete = list(self.objects_to_delete.keys())
        # new_object_meshes = self.new_object_meshes.copy()
        # self.new_object_meshes = []
        objects_labs = list(self.objects.keys())
        ...
        for label in objects_labs:
            obj=self.objects[label]
            if not label in scene.geometry:
                print( scene.geometry.keys())
                scene.add_geometry(obj.mesh, geom_name = obj.label)
                print('added object '+obj.name)
            else:
                obj.update_mesh()
                print('updated object '+obj.name)
