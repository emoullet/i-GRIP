...
class TargetDetector(Timed):
    ...
    def check_all_targets(self, timestamp = None):
        ...
        results = self.executor.map(self.update_target, target_labels)
    ...
    def update_target(self, target_label):
        if self.potential_targets[target_label].needs_update():
            impacts = self.potential_targets[target_label].update(self.ray_origins, self.ray_directions, self.timestamp)
        else:
            impacts = []    
        return impacts
...
class Target(Timed):
    ...
    def update_distance(self):
        ...
        t = time.time()
        closest = self.signed_distance_finder.nn(hand_pos_obj_frame.reshape(1,3))[0]
        distance_bis = np.linalg.norm(self.signed_distance_finder.vertices[closest]-hand_pos_obj_frame)
        print(f'distance bis :{distance_bis} ')
        print(f'closest time {(time.time()-t)*1000:.2f} ms')
        print(f'closest {closest}')
        coords = self.signed_distance_finder.vertices[closest]
    ...
    def check_impacts(self, ray_origins, ray_directions):
        ...
        try:
            impacts, _, _ = self.object.mesh.ray.intersects_location(ray_origins=ray_origins,
        ...
        return [impact for impact in impacts]
    ...
    def find_grip_adaptative(self):
        bounding_primitive = self.object.get_bounding_primitive()
