...
def check_impacts( data):
    ...
    if len(impacts)>0:
        impacts_scene_frame = [(trans@np.append(impact, 1))[:3] for impact in impacts]
    else:
        impacts_scene_frame = []
    return dict(label= label, object_frame_impacts = impacts, scene_frame_impacts = impacts_scene_frame)
...
class TargetDetector(Timed):
    ...
    def new_target(self, obj:ob.RigidObject):
        ...
        self.pool = mp.Pool(processes=len(self.potential_targets))
    ...
    def check_all_targets(self, timestamp = None):
        ...
        for label in target_labels:
            if self.does_target_need_update(label):
                target_data = dict(mesh = self.objects[label].mesh,
                                   trans = self.objects[label].get_mesh_transform(),
                                   inv_trans = self.objects[label].inv_mesh_transform, 
                                    ray_origins = self.ray_origins,
                                    ray_directions = self.ray_directions, 
                                    label = label)
                pool_data.append(target_data)
                self.potential_targets[label].update()
        results = self.pool.map(check_impacts, pool_data)
        
        all_impacts = []
        for result in results:
            if result is not None:
                all_impacts += result['scene_frame_impacts']
                self.potential_targets[result['label']].set_impacts(result['object_frame_impacts'])
