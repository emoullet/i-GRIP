...
class Target:
    ...
    def __init__(self, obj:ob.RigidObject, hand_label = None, impacts=None,  relative_hand_pos=None, analysis_window_size=10, visu_window_size = 40, index = 0) -> None:
        ...
        self.hand_position = manager.Position()
        self.object_position = manager.Position(self.object.get_position())
        self.obj_inv_transform = mp.Value(np.ndarray, self.object.inv_mesh_transform)
        self.relative_hand_pos = manager.Position()
    ...
    def data_processing_loop(self, mesh, process_event):
        while True:
            if process_event.is_set():
                
            relative_hand_pos = (self.obj_inv_transform.value@self.hand_position.get_ve())[:3]
    
            # update distance to target
            (_, distances, _) = mesh.nearest.on_surface(self.relative_hand_pos.reshape(1,3))
            distance = distances[0]
            self.distance_window.queue((distance, elapsed), time_type='elapsed')
            
            # update impact locations
            ray_origins_obj_frame = []
            impacts = []
            ray_origins_obj_frame = np.vstack([(self.obj_inv_transform.value@np.append(ray_origin, 1))[:3] for ray_origin in self.ray_origins])
            
            rot = self.obj_inv_transform.value[:3,:3]
            ray_directions_obj_frame = np.vstack([rot @ ray_dir for ray_dir in self.ray_directions])    
            try:
                impacts, _, _ = mesh.ray.intersects_location(ray_origins=ray_origins_obj_frame,
                                                                            ray_directions=ray_directions_obj_frame,
