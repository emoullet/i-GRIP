...
class Scene :
    ...
    def check_all_targets(self, scene, timestamp = None):
        ...
        tall = time.time()
        target_detector_labels = self.target_detectors.copy().keys()
        results = self.executor.map(self.check_target_detector, target_detector_labels)
        # results = self.executor.map(self.check_target, target_detector_labels, [timestamp]*len(target_detector_labels))
        for result in results:
            label, impacts = result
    ...
    def check_target_detector(self, label, timestamp = None):
        if timestamp is None:
            timestamp = time.time()
        impacts = self.target_detectors[label].check_all_targets()    
        return (label, impacts)
