...
class Target(Timed):
    ...
    def update_loop(self, update_event, update_done_event):
        while True:
            update_event.wait()
            self.set_timestamp(max(self.hand.timestamp, self.object.timestamp))
            t = time.time()
            print(f'update target loop {self.object.label} for hand {self.hand_label}')
            self.impacts = self.check_impacts(self.ray_origins, self.ray_directions)
    ...
    def check_impacts(self, ray_origins, ray_directions):
        ...
        if len(impacts)>0:
            impacts_scene_frame = [np.dot(self.object.get_mesh_transform(),np.append(impact, 1))[:3] for impact in impacts]
        else:
            impacts_scene_frame = []
